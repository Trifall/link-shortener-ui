<script>
	let { size = 64 } = $props(); // Default size of 256px

	// Calculate height based on original aspect ratio (116/256)
	let height = $derived(() => {
		const aspectRatio = 116 / 256;
		return Math.round(size * aspectRatio);
	});
</script>
	...
	><defs
		...
		><linearGradient id="e" x1="-33.2%" x2="91.7%" y1="100%" y2="0%"
			><stop offset="0%" stop-color="#D96504" stop-opacity=".8" /><stop
				...
				stop-color="#D96504"
				stop-opacity=".2"
			/><stop offset="100%" stop-color="#D96504" stop-opacity="0" /></linearGradient
